        ...
        if (this.readyState == 4 && this.status == 200) {
            ...
            if (obj.details.shared != "") {
                ...
                $("#post-list-large").append(
                    ...
                    '<button id=' + obj.id + ' class="see-Likes" onClick="seeLikesAndComments(this.id , ' + userName + ')"><h4 class="like-post">Likes - ' + obj.details.likes + '</h4>' +
                    '<button id=' + obj.id + ' class="see-Likes" onClick="seeLikesAndComments(this.id , ' + userName + ')"><h4 class="comment-post">Comments - ' + obj.details.comments + '</h4></button>' +
                    ...
                    ' class="card-form-button comment-button-large" name="comment-button-large" onclick="commentSinglePost(this.id , ' + userName + ')" >Comment</button>'
                );
            } else {
                $("#post-list-large").append(
...
function commentSinglePost(id, username) {
    const user = username;
...
$('.close').click(function () {
    ...
    xhttp.onreadystatechange = function () {
        ...
        if (this.readyState == 4 && this.status == 200) {
            ...
            } else if (window.location.href === 'http://localhost:4000/notifications') {
                window.location = "http://localhost:4000/notifications";
            } else {
                posts();
            }
